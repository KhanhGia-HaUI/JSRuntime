namespace Runtime.Modules.JavaScript
{
    public class Engine
    {
        public static void Evaluate(ref string Script_Directory, string[] args)
        {
            ...
            engine.SetValue("DotNetPlatform", new Runtime.Modules.Standards.Platform());
            ...
            engine.SetValue("DotNetLocalization", new Runtime.Modules.Standards.Localization());

            try
            {
                engine.Execute(fs.ReadText(main_js, Standards.IOModule.EncodingType.UTF8), main_js);
            }
            catch (Exception ex)
            {
